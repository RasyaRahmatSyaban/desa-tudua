...
<div class="space-y-6">
    <form action="{{ route('admin.pelayanan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Nama -->
            <div>
                <label for="nama_layanan" class="block text-sm font-medium text-gray-700 mb-2">Nama Layanan <span class="text-red-500">*</span></label>
                <input type="text" name="nama_layanan" id="nama_layanan"
                    ...
                    @error('nama_layanan') border-red-500 @enderror 
            ...
            <div>
                ...
                <select name="kategori" id="kategori" @error('kategori') border-red-500 @enderror 
                    ...
                    <option value="Dokumen Identitas" {{ old('kategori') == 'Dokumen Identitas' ? 'selected' : '' }}>Dokumen Identitas</option>
                    <option value="Kependudukan" {{ old('kategori') == 'Kependudukan' ? 'selected' : '' }}>Kependudukan</option>
                    <option value="Pencatatan Sipil" {{ old('kategori') == 'Pencatatan Sipil' ? 'selected' : '' }}>Pencatatan Sipil</option>
                </select>
                @error('kategori')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <!-- Link -->
            <div>
                <label for="link_google_form" class="block text-sm font-medium text-gray-700 mb-2">Link Google Form<span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <input type="text" name="deskripsi" id="deskripsi"
                    ...
                    placeholder="Layanan ini merupakan layanan" required>
                @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
