...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200">
            <div class="p-6">
                <form method="POST" action="{{ route('admin.dana-keluar.update', $danaKeluar->id) }}" class="">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <select id="bulan" name="bulan"
                                    ...
                                    @foreach(['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'] as $i => $bln)
                                        <option value="{{ $i + 1 }}" {{ old('bulan', $danaKeluar->bulan) == $i + 1 ? 'selected' : '' }}>
                                            {{ $bln }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" id="tahun" name="tahun"
                                    value="{{ old('tahun', $danaKeluar->tahun ?? date('Y')) }}" min="1900" max="2100"
                                    class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200"
                                    required>
                                @error('tahun')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="">
                                ...
                                <select id="kategori" name="kategori"
                                    ...
                                    <option value="Operasional" {{ old('kategori', $danaKeluar->kategori) == 'Operasional' ? 'selected' : '' }}>
                                        Operasional
                                    </option>
                                    <option value="Sosial" {{ old('kategori', $danaKeluar->kategori) == 'Sosial' ? 'selected' : '' }}>
                                        Sosial
                                    </option>
                                    <option value="Kesehatan" {{ old('kategori', $danaKeluar->kategori) == 'Kesehatan' ? 'selected' : '' }}>
                                        Kesehatan
                                    </option>
                                    <option value="Pendidikan" {{ old('kategori', $danaKeluar->kategori) == 'Pendidikan' ? 'selected' : '' }}>
                                        Pendidikan
                                    </option>
                                    <option value="Lainnya" {{ old('kategori', $danaKeluar->kategori) == 'Lainnya' ? 'selected' : '' }}>
                                        Lainnya
                                    </option>
                                </select>
                                @error('kategori')
                    ...
                    <div class="space-y-4 pt-6">
                        <div class="space-y-2">
                            ...
                            <textarea id="keterangan" name="keterangan" rows="4"
                                ...
                                class="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 resize-none">{{ old('keterangan', $danaKeluar->keterangan) }}</textarea>
                            @error('keterangan')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
