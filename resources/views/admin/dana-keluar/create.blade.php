...
@section('page-description', 'Tambah data dana keluar')
                ...
                <form action="{{ route('admin.dana-keluar.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="bulan" name="bulan"
                                ...
                                @foreach(['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'] as $i => $bln)
                                    <option value="{{ $i + 1 }}" {{ old('bulan') == $i + 1 ? 'selected' : '' }}>{{ $bln }}
                                    </option>
                                @endforeach
                            </select>
                            @error('bulan')
                        ...
                        <div>
                            ...
                            <input type="number" id="tahun" name="tahun" value="{{ old('tahun', date('Y')) }}" min="1900"
                                max="2100"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number"
                                    ...
                                    @error('jumlah') border-red-500 @enderror id="jumlah" name="jumlah"
                                    value="{{ old('jumlah') }}" placeholder="0" min="0" step="1000" required>
                            </div>
                            @error('jumlah')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                @error('kategori') border-red-500 @enderror id="kategori" name="kategori">
                    ...
                    <div class="mt-6">
                        ...
                        <textarea
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            @error('keterangan') border-red-500 @enderror id="keterangan" name="keterangan" rows="4"
